...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getStaffPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'staff');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.staff.count(countOptions as unknown),
  ...
  async function getStaff() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.staff
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'staff'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
